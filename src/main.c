...
double* toom_cook_with_naive_base(double* A, int degA, double* B, int degB, int k) {
    return toom_cook_polynomial_multiplication(A, degA, B, degB, k, BASE_NAIVE);
}

double* toom_cook_4_with_naive_base(double* A, int degA, double* B, int degB, int k) {
    return toom_4_polynomial_multiplication(A, degA, B, degB, k, BASE_NAIVE);
}
...
static int main_demo_original(void) {
    ...
    for (int k = 2; k <= 4; k++) {
        run_algorithm(A, degA, B, degB, toom_cook_with_naive_base, "Toom-Cook Polynomial Multiplication", k);
    }
    ...
    for (int k = 2; k <= 4; k++) {
        run_algorithm(A, degA, B, degB, toom_cook_4_with_naive_base, "Toom-4 Polynomial Multiplication", k);
    }
